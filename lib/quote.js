...
async function draftFromRfq(rfq) {
  ...
  const supplierState = (issuer.type === 'dealer' && issuer.state_code) || s['company.state_code'] || '27';
  ...
  const q = {
    ...
    customer: rfq.customer, project: rfq.project, raised_by: rfq.raised_by || null, line: rfq.line, issuer,
    items, freight: 0, freight_hsn: '996511', freight_gst: 18, packing: 0,
    terms: { payment: s['quote.terms_payment'], delivery: issuer.type === 'dealer' ? String(s['quote.terms_delivery'] || '').replace(/Ex-works Satara\.?\s*/i, 'Ex-works. ') : s['quote.terms_delivery'], warranty: s['quote.terms_warranty'],
             other: currency === 'USD' ? (issuer.type === 'dealer' ? String(s['quote.terms_export'] || '').replace(/EXW Satara/i, 'EXW') : s['quote.terms_export']) : s['quote.terms_other'] },
  ...
  if (user && user.role === 'dealer') {
    ...
    if (d && d.status === 'approved') {
      ...
      return { type: 'dealer', dealer: d.email, code: d.code, company: d.company, contact: d.contact, email: d.email, phone: d.phone, addr1: d.addr1, addr2: d.addr2, city: d.city, pincode: d.pincode,
        ...
        max_discount_pct: t.max_discount_pct, billing_discount_pct: t.discount_pct };
    }
  }
...
function docParties(q, s) {
  ...
  if (is.type === 'dealer') {
    const company = { name: is.company, addr1: is.addr1, addr2: is.addr2, city: is.city, pincode: is.pincode, state_name: is.state_name, country: is.country, email: is.email, phone: is.phone, web: is.web, gstin: is.gstin, iec: is.iec, contact: is.contact,
      dealer: true, footer: is.quote_footer || (s['dealer.show_manufacturer'] === '1' ? `Products manufactured by ${s['company.name'] || 'ADONI TECH'}, India` : '') };
...
function limitCheck(q, maxPct) {
  ...
  for (const i of q.items.filter(PRICED)) {
    ...
    if (eff > max + 0.01) v.push({ seq: i.seq, model: i.model, list, net: Math.round(net * 100) / 100, floor: Math.round(list * (1 - max / 100) * 100) / 100, discount_pct: Math.round(eff * 10) / 10 });
  }
  const sp = q.special;
  const approved = sp && sp.status === 'approved' && v.every(x => { const a = (sp.approved_nets || {})[x.seq]; return a != null && x.net >= a - 0.01; });
  return { ok: !v.length || !!approved, max_pct: max, violations: v, special_approved: !!approved };
}
/** What ADONI TECH bills the dealer: list - billing discount, per priced line (INR or USD as quoted). */
function dealerBilling(q) {
  const is = q.issuer || {}; if (is.type !== 'dealer') return null;
  const sp = q.special && q.special.status === 'approved' && q.special.billing_discount_pct != null && q.special.billing_discount_pct !== '' ? Number(q.special.billing_discount_pct) : null;
  const d = sp != null ? sp : Number(is.billing_discount_pct || 25);
  ...
  return { list_value: Math.round(list * 100) / 100, customer_value: Math.round(customer * 100) / 100, billing_discount_pct: d, billing_value: billing, dealer_margin: Math.round((customer - billing) * 100) / 100, currency: q.currency, special_billing: sp != null };
}
...
async function list() {
  ...
  for (const k of keys) { const q = await store.getJSON(k); if (q) out.push({ id: q.id, number: q.number, rev: q.rev, date: q.date, status: q.status, currency: q.currency,
    ...
    issuer: q.issuer ? (q.issuer.type === 'dealer' ? 'dealer:' + q.issuer.company : 'adoni:' + (q.issuer.office || '')) : 'adoni', dealer: q.issuer && q.issuer.type === 'dealer' ? q.issuer.dealer : null, raised_by: q.raised_by && q.raised_by.email, special: q.special && q.special.status }); }
  return out.sort((a, b) => (b.number + b.rev).localeCompare(a.number + a.rev));
}

module.exports = { draftFromRfq, totals, leadTime, get, save, list, idOf, tables, issuerFor, docParties, limitCheck, dealerBilling, offices, officeFor };
