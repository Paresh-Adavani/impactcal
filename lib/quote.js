...
async function priceLine(item, tables) {
  ...
  if (t === 'wire_rope_isolators') {
    ...
    return { description: [item.model, p ? `Ø${p.family_wire_mm} mm cable, ${p.c_load_n} N rated (compression)` : null, item.remark].filter(Boolean).join(' · '),
             hsn: p ? p.hsn : '84879000', uom: 'NOS', rate_inr: p && p.price_inr != null ? p.price_inr : 0, gst_rate: p ? p.gst_rate : 18, lead_time_days: p ? p.lead_time_days : null, ga_pdf: p ? p.ga_pdf : '', weight_kg: p ? p.weight_kg : null };
  }
  if (t === 'rubber_mounts') {
...
async function draftFromRfq(rfq) {
  ...
  for (const it of rfq.items || []) {
    ...
    items.push({ seq: ++seq, kind: 'product', table: it.table || 'shock_absorbers', key: it.key || it.bk || it.model, model: it.model,
      ...
      discount_pct: 0, gst_rate: pr.gst_rate, lead_time_days: pr.lead_time_days, ga_pdf: pr.ga_pdf, weight_kg: pr.weight_kg != null && pr.weight_kg !== '' ? Number(pr.weight_kg) : null, mounting: it.mounting || '', cap: it.cap || '' });
    if (it.mounting) {
...
function freightCalc(q, s) {
  const rate = Number((s && s['freight.rate_per_kg']) != null && s['freight.rate_per_kg'] !== '' ? s['freight.rate_per_kg'] : 35);
...
module.exports = { freightCalc, draftFromRfq, totals, leadTime, get, save, list, idOf, tables, issuerFor, docParties, limitCheck, dealerBilling, offices, officeFor };
