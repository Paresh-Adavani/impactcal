...
async function verifyOtp(email, code, settings) {
  ...
  const role = roleFor(e, settings);
  const user = { email: e, role, name: e.split('@')[0] };
  const users = (await store.getJSON('users/index')) || {};
  users[e] = { ...(users[e] || {}), email: e, role, last_login: new Date().toISOString(), logins: ((users[e] || {}).logins || 0) + 1 };
...
const canSeePrices = u => !!u && (u.role === 'admin' || u.role === 'sales');
...
module.exports = { sign, verify, roleFor, requestOtp, verifyOtp, optional, requireRole, canSeePrices, actionLink, verifyAction, ADMIN_EMAILS };
