...
async function raw(table) {
  ...
  /* one-time correction: the firm is ADONI TECH (proprietorship), never "PVT LTD".
   * An old settings upload carried the wrong name; fix it in place and keep the fix. */
  if (table === 'settings' && /ADONI\s*TECH\s*PVT\.?\s*LTD\.?/i.test(text)) {
    text = text.replace(/ADONI\s*TECH\s*PVT\.?\s*LTD\.?/gi, 'ADONI TECH');
    if (source === 'upload') { try { await store.set('csv/settings.csv', text); await store.audit('system', 'settings.fix_company_name', 'settings', 'company name corrected to ADONI TECH (was PVT LTD)'); } catch {} }
  }
  memo.set(table, { at: Date.now(), text, source });
  return text;
}
function bust(table) { memo.delete(table); memo.delete('lt:' + table); }

/* Lead-time overrides: store leadtimes/<table> = { key: { days, by, at, note } }.
 * Kept apart from the CSV so a later deploy or CSV upload never wipes what the team set. */
const LT_TABLES = ['shock_absorbers', 'accessories', 'wire_rope_isolators', 'rubber_mounts', 'other_products'];
async function leadOverrides(table) {
  if (!LT_TABLES.includes(table)) return {};
  const m = memo.get('lt:' + table); if (m && Date.now() - m.at < 60000) return m.v;
  const v = (await store.getJSON('leadtimes/' + table)) || {};
  memo.set('lt:' + table, { at: Date.now(), v }); return v;
}
async function setLeadTimes(table, changes, by) {
  if (!LT_TABLES.includes(table)) throw Object.assign(new Error('no lead times in ' + table), { status: 400 });
  const cur = (await store.getJSON('leadtimes/' + table)) || {}; const at = new Date().toISOString(); let n = 0;
  for (const c of changes) {
    const k = String(c.key);
    if (c.days === null || c.days === '' || c.days === undefined) { if (cur[k]) { delete cur[k]; n++; } continue; }
    const d = Number(c.days); if (!Number.isInteger(d) || d < 0 || d > 730) throw Object.assign(new Error(`lead time for ${k} must be whole days 0 - 730`), { status: 400 });
    cur[k] = { days: d, by, at, note: String(c.note || '').slice(0, 200) }; n++;
  }
  await store.setJSON('leadtimes/' + table, cur); memo.delete('lt:' + table); return n;
}

async function rows(table, opts = {}) {
  const r = csv.parse(await raw(table)).rows;
  if (!opts.base && LT_TABLES.includes(table)) {
    const lt = await leadOverrides(table); const key = TABLES[table].key;
    if (Object.keys(lt).length) for (const x of r) { const o = lt[x[key]]; if (o) x.lead_time_days = String(o.days); }
  }
  // settings saved from the admin panel win, but keys added to data/settings.csv in a later release still apply
  if (table === 'settings' && memo.get(table).source === 'upload') {
    const have = new Set(r.map(x => x.key));
    try { for (const d of csv.parse(fs.readFileSync(path.join(DATA_DIR, 'settings.csv'), 'utf8')).rows) if (d.key && !have.has(d.key)) r.push(d); } catch {}
  }
  return r;
}
...
module.exports = { LT_TABLES, leadOverrides, setLeadTimes, TABLES, raw, rows, header, source, bust, products, accessories, wri, rubber, settings, gaIndex, i18n, company, bank,
