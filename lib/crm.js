...
function leadFromRfq(rfq, settings, publicUrl) {
  ...
  return {
    ...
    landmark: '',
    gstin: c.gstin || '',
    remarks: [`RFQ ${rfq.number}`, rfq.issuer && rfq.issuer.type === 'dealer' ? `Via dealer ${rfq.issuer.company} (${rfq.issuer.code})` : '', summary, rfq.message, rfq.raised_by ? `Raised by ${rfq.raised_by.name || rfq.raised_by.email}` : 'Raised by customer on web', publicUrl ? `${publicUrl}/admin.html#${rfq.number && rfq.number.includes('/S/') ? 'selections' : 'rfq=' + rfq.id}` : ''].filter(Boolean).join(' | ').slice(0, 1000),
