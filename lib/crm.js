...
function leadFromRfq(rfq, settings, publicUrl) {
  ...
  return {
    ...
    landmark: '',
    gstin: c.gstin || '',
    remarks: [`RFQ ${rfq.number}`, c.customer_id ? `Customer ${c.customer_id} (already in CRM)` : '', rfq.issuer && rfq.issuer.type === 'dealer' ? `Via dealer ${rfq.issuer.company} (${rfq.issuer.code})` : '', summary, rfq.message, rfq.raised_by ? `Raised by ${rfq.raised_by.name || rfq.raised_by.email}` : 'Raised by customer on web', publicUrl ? `${publicUrl}/admin.html#${rfq.number && rfq.number.includes('/S/') ? 'selections' : 'rfq=' + rfq.id}` : ''].filter(Boolean).join(' | ').slice(0, 1000),
...
/** Report download (no RFQ yet) -> lead with a "report" source. Never throws. */
async function selectionDownloaded(sel, settings, publicUrl) {
  if (settings['crm.enabled'] !== '1') { await store.audit('system', 'crm.skipped', sel.number, 'crm.enabled is not 1'); return { ok: false, reason: 'CRM disabled' }; }
  const lead = leadFromRfq(sel, settings, publicUrl);
  lead.source = (settings['crm.source_report'] || (settings['crm.source'] || 'ImpactCal Web') + ' (report)');
  lead.requirement = `Report download ${sel.number}: ${(sel.items || []).map(i => `${i.model} x ${i.qty}`).join(', ')}`.slice(0, 250);
  lead.remarks = (`Selection report downloaded, no RFQ yet. ` + lead.remarks).slice(0, 1000);
  const r = await pushLead(lead, sel.number);
  return { ...r, lead };
}
...
module.exports = { rfqCreated, selectionDownloaded, pushLead, leadFromRfq, status, configured };
