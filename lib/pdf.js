...
function render(q, company, bank, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    /* header: ADONI TECH (office of the sales person) or the dealer's own letterhead */
    const isDealer = !!company.dealer;
    const logo = opts.logoPath || path.join(__dirname, '..', 'site', 'img', 'logo.png');
    if (!isDealer && fs.existsSync(logo)) { try { doc.image(logo, L, y, { height: 34 }); } catch {} }
    let dealerLogo = false;
    if (isDealer && company.logo_buffer) { try { doc.image(company.logo_buffer, L, y, { fit: [170, 38] }); dealerLogo = true; } catch {} }
    if (isDealer && !dealerLogo) doc.font('Helvetica-Bold').fontSize(15).fillColor(NAVY).text(company.name || '', L, y + 6, { width: W - 170 });
    ...
    const exp = q.currency !== 'INR';
    const hl = isDealer
      ? [company.name, [company.addr1, company.addr2, company.city, company.pincode, company.state_name, company.country && company.country !== 'India' ? company.country : 'India'].filter(Boolean).join(', '),
         [company.contact, company.email, company.phone, company.web].filter(Boolean).join(' · '),
         [company.gstin ? 'GSTIN ' + company.gstin : '', exp && company.iec ? 'IEC ' + company.iec : ''].filter(Boolean).join('   ')]
      : [`${company.name || 'ADONI TECH'}${company.iso ? ' · ' + company.iso : ''}${company.office_label ? ' · ' + company.office_label : ''}`,
         `${company.addr1 || ''}, ${company.addr2 ? company.addr2 + ', ' : ''}${company.city || ''} ${company.pincode || ''}, ${company.state_name || ''}, India`,
         company.works ? `Works: ${company.works}` : '',
         [company.person_email || company.email, company.phone, company.web].filter(Boolean).join(' · ') + `   GSTIN ${company.gstin || ''}${exp && company.iec ? '   IEC ' + company.iec : ''}`,
         company.channel_partner ? `Channel partner: ${[company.channel_partner.company, company.channel_partner.contact, company.channel_partner.phone, company.channel_partner.email].filter(Boolean).join(' · ')}` : ''];
    doc.fontSize(8.5).fillColor('#222').font('Helvetica');
    let hy = y; for (const t of hl.filter(Boolean)) { doc.text(t, L, hy, { width: W }); hy += 11; }
    y = hy + 8;
    ...
    const terms = [
      ...
      ['Payment', T.payment], ['Delivery', T.delivery], ['Warranty', T.warranty], ['Other', T.other],
      q.currency !== 'INR' && company.iec ? ['Exporter', `${company.name}, IEC ${company.iec}${company.gstin ? ', GSTIN ' + company.gstin : ''}`] : null, q.notes ? ['Notes', q.notes] : null,
    ].filter(x => x && x[1]);
    ...
    /* bank */
    const hasBank = bank && Object.values(bank).some(Boolean);
    if (company.dealer && !hasBank) { /* dealer gave no bank details: leave the block out */ } else {
    ...
    /* signature */
    doc.font('Helvetica').fontSize(8.5).fillColor('#222').text(`For ${company.name || 'ADONI TECH'}`, L + W - 200, y, { width: 200, align: 'right' });
    doc.text(q.approved_by ? (isDealer ? q.approved_by : `Approved by ${q.approved_by}`) : 'Authorised signatory', L + W - 200, y + 34, { width: 200, align: 'right' });
    doc.fontSize(7.5).fillColor(GREY).text(isDealer ? (company.footer || 'E & O E.') : 'This quotation is generated from ImpactCal after engineering review of the application data supplied. E & O E.', L, y + 34, { width: W - 210 });
    doc.end();
  });
}
