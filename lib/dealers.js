...
const PROFILE_FIELDS = ['company', 'contact', 'phone', 'addr1', 'addr2', 'city', 'pincode', 'state_code', 'state_name', 'country', 'gstin', 'pan', 'iec', 'web', 'bank_beneficiary', 'bank_name', 'bank_branch', 'bank_account', 'bank_ifsc', 'bank_swift', 'quote_footer'];
...
function clean(profile) {
  const out = {};
  for (const f of PROFILE_FIELDS) if (profile[f] !== undefined) out[f] = String(profile[f] ?? '').trim().slice(0, 300);
  if (out.gstin) {
