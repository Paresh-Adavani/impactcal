...
let blobs = null;
function onNetlify() { return !!(process.env.NETLIFY || process.env.NETLIFY_BLOBS_CONTEXT || process.env.NETLIFY_DEV); }
...
function connect(event) {
  ...
  try {
    ...
    blobs = b.getStore({ name: 'impactcal', consistency: 'strong' });
  } catch (e) { console.error('blobs unavailable, falling back to disk:', e.message); blobs = null; }
}

const DIR = process.env.IMPACTCAL_STORE || path.join(__dirname, '..', 'data', '_store');
...
module.exports = { connect, onNetlify, getJSON, setJSON, getText, getBuffer, set, del, list, nextNumber, fyCode, audit, DIR };
