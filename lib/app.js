...
/* ------------------------------------------------------------------ reference */
app.get('/health', wrap(async (_q, r) => r.json({ ok: true, products: (await data.products()).length, store: store.onNetlify() ? 'netlify-blobs' : 'disk', time: new Date().toISOString() })));
...
app.post('/rfq', auth.optional, wrap(async (req, res) => {
  ...
  const rfq = { id: idOf(number), number, created_at: new Date().toISOString(), status: 'new', line: b.line || 'shock',
    customer: c, project: b.project || {}, raised_by: req.user ? { email: req.user.email, role: req.user.role } : null,
  ...
  res.json({ id: rfq.id, number, mail: mails.sales, ack: mails.customer, quotation: q ? q.number : null, alert: alert && alert.mail ? alert.mail.ok : false });
}));
...
app.get('/admin/rfqs', staff, wrap(async (req, res) => {
  ...
  for (const k of keys) { const r = await store.getJSON(k); if (!r) continue; if (req.user.role === 'sales' && r.raised_by && r.raised_by.email !== req.user.email && r.raised_by.role !== 'customer') { /* sales see own + web */ }
    out.push({ id: r.id, number: r.number, created_at: r.created_at, status: r.status, line: r.line, customer: r.customer && (r.customer.company || r.customer.name), country: r.customer && r.customer.country, items: r.items.length, quotation_id: r.quotation_id, raised_by: r.raised_by && r.raised_by.email }); }
