...
app.get('/products', auth.optional, wrap(async (req, res) => {
  ...
  res.json(rows.map(r => publicRubber(r, req.user)));
}));
...
app.get('/wri/data', auth.optional, wrap(async (req, res) => {
  ...
  const lug_options = quote.lugOptions(settings).map(o => show ? o : { label: o.label, default: o.default });   // surcharge % only for price viewers
  res.json({ awri, prices, prices_visible: show, lug_options, wire_options: quote.wireOptions(settings), fx: rate ? { inr_per_usd: rate.rate, uplift_pct: Number(settings['fx.uplift_pct'] || 0) } : null });
}));

/* ------------------------------------------------------------------ rubber mounts */
// supplier cross-references (source column) stay inside ADONI TECH: admin and sales only
const isStaff = u => !!(u && (u.role === 'admin' || u.role === 'sales'));
function publicRubber(p, user) {
  const { price_inr, source, ...r } = p;
  if (auth.canSeePrices(user)) r.price_inr = price_inr;
  if (isStaff(user)) r.source = source;
  return r;
}
...
app.post('/rubber/select', auth.optional, wrap(async (req, res) => {
  ...
  out.candidates = out.candidates.map(c => ({ ...c, product: publicRubber(c.product, req.user) }));
  res.json(out);
}));
...
async function createRfq(b, user) {
  ...
  const rfq = { id: idOf(number), number, created_at: new Date().toISOString(), status: 'new', line: b.line || 'shock',
    ...
    issuer, ...(dealer && b.route === 'direct' ? { route: 'direct' } : {}), selection: b.selection || null, items: b.items.map(i => ({ table: i.table || 'shock_absorbers', key: i.key || i.bk || i.model, model: i.model, qty: Number(i.qty) || 1, mounting: i.mounting || '', cap: i.cap || '', damping_code: i.damping_code || '', remark: i.remark || '', ...(i.lug ? { lug: String(i.lug).slice(0, 60) } : {}), ...(i.wire ? { wire: String(i.wire).slice(0, 60) } : {}) })),
...
app.get('/approve/:id', approveAuth, wrap(async (req, res) => {
  ...
  res.json({ quotation: q, rfq, drawings, files: (await store.getJSON('qfiles/' + q.id)) || [], bank: parties.bank, company: parties.company, drawings_mode: settings['drawings.mode'] || 'attach',
    access: req.access, actor: req.actor, limit: limitFor(q, req.access), billing: req.access !== 'sales' ? quote.dealerBilling(q, settings) : null, offices,
...
app.put('/approve/:id', approveAuth, wrap(async (req, res) => {
  ...
  const direct = q.route === 'direct';
  // direct supply is ADONI TECH's own offer: the dealer sets price & discount only; freight follows the weight rule
  const fields = direct && !isAdmin ? ['valid_days', 'notes'] : ['valid_days', 'freight', 'freight_hsn', 'freight_gst', 'packing', 'notes', 'place_of_supply_code', 'supply_type', 'lead_time_override'];
  for (const k of fields) if (b[k] !== undefined) q[k] = b[k];
  if (direct && isAdmin && b.freight !== undefined) { q.freight_locked = Number(b.freight) !== Number((q.freight_calc || {}).amount); if (q.freight_locked) q.freight_label = 'Packing & freight'; }
  if (direct && !isAdmin && Array.isArray(b.items) && b.items.some(i => !q.items.find(x => x.seq === i.seq))) return res.status(400).json({ error: 'a direct-supply quotation carries ADONI TECH catalogue lines only - extra lines are not possible' });
  if (b.terms && !(direct && !isAdmin)) q.terms = { ...q.terms, ...b.terms };   // ADONI TECH terms on a direct-supply offer
...
app.post('/approve/:id/send', approveAuth, wrap(async (req, res) => {
  ...
  /* own uploaded files (customer drawings, STEP/DWG/CAD, PDFs) always travel with the offer */
  const extras = [];
  for (const f of (await store.getJSON('qfiles/' + q.id)) || []) {
    const b = await store.getBuffer('qfile/' + q.id + '/' + f.filename);
    if (b) extras.push({ filename: f.filename, buffer: b }); else missing.push(f.filename);
  }
  const selfSend = req.access === 'dealer' && req.body && req.body.self_send;
  const m = selfSend ? { ok: true, to: 'sent by the dealer from his own e-mail', self: true } : await notify.offerSent(q, settings, company, buf, attach, links, extras);
  if (m.ok) { q.status = 'sent'; q.sent_at = new Date().toISOString(); q.sent_to = m.to; }
  q.send_result = m; q.drawings_missing = missing;
  const billing = quote.dealerBilling(q, settings);
  if (billing) q.billing = billing;
  if (billing && billing.route === 'direct' && m.ok && !q.commission) q.commission = { status: 'quoted', history: [{ status: 'quoted', at: new Date().toISOString(), by: q.sent_by }] };
  await quote.save(q, settings);
  ...
  res.json({ ok: m.ok, reason: m.reason, sent_to: m.to, attached: [...attach, ...extras].map(a => a.filename), links, missing, billing: req.access !== 'sales' ? billing : null, internal: internal ? internal.ok : null });
}));

/* ---- own files on a quotation: customer drawings, 3-D models (STEP/IGES), DWG/DXF, PDFs ---- */
const QF_EXT = /\.(pdf|step|stp|igs|iges|dwg|dxf|zip|png|jpe?g|docx?|xlsx?|sldprt|x_t|sat|3dm)$/i;
const qfSafe = n => String(n || '').split(/[\\/]/).pop().replace(/[^A-Za-z0-9 ._()\-]/g, '_').slice(0, 120);
async function qfSave(q, name, buf, req) {
  await store.set('qfile/' + q.id + '/' + name, buf);
  const idx = (await store.getJSON('qfiles/' + q.id)) || [];
  const at = new Date().toISOString(); const by = req.actor ? req.actor.email : 'approval-link';
  const e = idx.find(x => x.filename === name);
  if (e) Object.assign(e, { bytes: buf.length, by, at }); else idx.push({ filename: name, bytes: buf.length, by, at });
  await store.setJSON('qfiles/' + q.id, idx);
  await store.audit(by, 'quotation.file_add', q.number, `${name} (${(buf.length / 1024).toFixed(0)} kB)`);
  return idx;
}
app.post('/approve/:id/file', approveAuth, wrap(async (req, res) => {   // { filename, base64 } or chunked { filename, upload_id, index, total, base64 }
  const q = await quote.get(req.params.id); if (!q) return res.status(404).json({ error: 'not found' });
  const { filename, upload_id: uid, index, total, base64 } = req.body || {};
  const name = qfSafe(filename);
  if (!name || !base64) return res.status(400).json({ error: 'filename and base64 required' });
  if (!QF_EXT.test(name)) return res.status(400).json({ error: 'file type not supported - PDF, STEP/STP, IGS/IGES, DWG/DXF, ZIP, images or Office files please' });
  const i = Number(index) || 0, n = Number(total) || 1;
  if (!(n >= 1 && n <= 12) || !(i >= 0 && i < n)) return res.status(400).json({ error: 'bad chunk numbers' });
  const part = Buffer.from(base64, 'base64');
  if (part.length > 4 * 1024 * 1024) return res.status(400).json({ error: 'chunk above 4 MB' });
  if (n === 1) { const files = await qfSave(q, name, part, req); return res.json({ ok: true, filename: name, bytes: part.length, files }); }
  const key = `qtmp/${q.id}/${String(uid || '').replace(/[^a-z0-9]/gi, '').slice(0, 40)}`;
  if (key.endsWith('/')) return res.status(400).json({ error: 'upload_id required for chunked upload' });
  await store.set(`${key}/${i}`, part);
  if (i < n - 1) return res.json({ ok: true, part: i });
  const parts = [];
  for (let k = 0; k < n; k++) { const b = await store.getBuffer(`${key}/${k}`); if (!b) return res.status(400).json({ error: `part ${k} missing - please upload the file again` }); parts.push(b); }
  const buf = Buffer.concat(parts);
  for (let k = 0; k < n; k++) await store.del(`${key}/${k}`).catch(() => {});
  if (buf.length > 20 * 1024 * 1024) return res.status(400).json({ error: 'file above 20 MB - too big to e-mail; please share it as a link instead' });
  const files = await qfSave(q, name, buf, req);
  res.json({ ok: true, filename: name, bytes: buf.length, files });
}));
app.delete('/approve/:id/file', approveAuth, wrap(async (req, res) => {
  const q = await quote.get(req.params.id); if (!q) return res.status(404).json({ error: 'not found' });
  const name = qfSafe(req.query.name);
  await store.del('qfile/' + q.id + '/' + name).catch(() => {});
  const idx = ((await store.getJSON('qfiles/' + q.id)) || []).filter(x => x.filename !== name);
  await store.setJSON('qfiles/' + q.id, idx);
  await store.audit(req.actor ? req.actor.email : 'approval-link', 'quotation.file_remove', q.number, name);
  res.json({ ok: true, files: idx });
}));
...
/* delete trial / test records (admin only, audited). An RFQ takes its draft quotation with it. */
app.delete('/admin/rfq/:id', admin, wrap(async (req, res) => {
  const r = await store.getJSON('rfq/' + req.params.id); if (!r) return res.status(404).json({ error: 'not found' });
  if (r.quotation_id) await store.del('quotation/' + r.quotation_id).catch(() => {});
  await store.del('rfq/' + r.id);
  await store.audit(req.user.email, 'rfq.delete', r.number, 'deleted' + (r.quotation_id ? ' with quotation ' + r.quotation_id.replace(/-/g, '/') : ''));
  res.json({ ok: true, deleted: r.number, quotation: r.quotation_id || null });
}));
app.delete('/admin/selection/:id', admin, wrap(async (req, res) => {
  const r = await store.getJSON('selection/' + req.params.id); if (!r) return res.status(404).json({ error: 'not found' });
  await store.del('selection/' + r.id);
  await store.audit(req.user.email, 'selection.delete', r.number, 'deleted' + (r.rfq_number ? ' (had RFQ ' + r.rfq_number + ', kept)' : ''));
  res.json({ ok: true, deleted: r.number });
}));
app.delete('/admin/quotation/:id', admin, wrap(async (req, res) => {
  const q = await quote.get(req.params.id); if (!q) return res.status(404).json({ error: 'not found' });
  await store.del('quotation/' + q.id);
  if (q.rfq_id) { const r = await store.getJSON('rfq/' + q.rfq_id); if (r && r.quotation_id === q.id) { r.quotation_id = null; if (r.status === 'draft' || r.status === 'quoted') r.status = 'new'; await store.setJSON('rfq/' + r.id, r); } }
  await store.audit(req.user.email, 'quotation.delete', q.number, 'deleted (was ' + q.status + ')');
  res.json({ ok: true, deleted: q.number });
}));
...
app.post('/dealer/apply', loggedIn, wrap(async (req, res) => {
  ...
  let welcome = null;   // first application: the standard pricing policy goes to the applicant (final terms follow on approval)
  if (!before) { try { welcome = await notify.dealerPolicy(d, dealers.rules(d, settings), { provisional: true }); await store.audit('system', 'dealer.policy.standard', d.company, welcome.ok ? 'sent' : welcome.reason); } catch (e) { console.error(e); } }
  await store.audit(req.user.email, before ? 'dealer.update' : 'dealer.apply', d.company, d.gstin || '');
  res.json({ ok: true, dealer: d, mail: m ? m.ok : null, policy_mail: welcome ? welcome.ok : null });
}));
app.get('/dealer/me', loggedIn, wrap(async (req, res) => {
  const d = await dealers.get(req.user.email); const settings = await data.settings();
  res.json({ dealer: d, terms: d ? dealers.terms(d, settings) : null, rules: d ? dealers.rules(d, settings).map(({ own, standard, ...r }) => r) : null, policy: d && d.policy ? { version: d.policy.version, confirmed_at: d.policy.confirmed_at } : null, role: req.user.role, relogin: !!(d && d.status === 'approved' && req.user.role !== 'dealer') });
}));
...
/* customer master: sales / admin share ADONI TECH's list (new customers go to the CRM); a dealer keeps his own */
const customers = require('./customers');
async function customerCrm(c, user, settings) {
  if (customers.scopeOf(user) !== 'adoni') return null;   // a dealer's customer never goes to the ADONI TECH CRM
  if (settings['crm.enabled'] !== '1') { await customers.mark(c.id, { crm: { ok: false, reason: 'CRM disabled in settings (crm.enabled)', at: new Date().toISOString() } }); return { ok: false, reason: 'CRM disabled in settings (crm.enabled)' }; }
  const r = await crm.pushLead(customers.lead(c, settings, auth.displayName(user.email, settings)), c.id);
  await customers.mark(c.id, { crm: { ok: r.ok, status: r.status, reason: r.reason, at: new Date().toISOString() } });
  return r;
}
async function customerPrefix(user) { if (user.role !== 'dealer') return 'C'; const d = await dealers.get(user.email); return ((d && d.code) || 'DLR') + '-C'; }
app.get('/customers', portalUser, wrap(async (req, res) => res.json(await customers.list(req.user, req.query.q))));
app.post('/customers', portalUser, wrap(async (req, res) => {
  const settings = await data.settings();
  const c = await customers.create(req.user, req.body || {}, await customerPrefix(req.user));
  const r = await customerCrm(c, req.user, settings);
  await store.audit(req.user.email, 'customer.create', c.id, `${c.company || c.contact}${r ? ' · CRM ' + (r.ok ? 'pushed' : 'not pushed: ' + (r.reason || r.status)) : ''}`);
  res.json({ ok: true, customer: await customers.get(req.user, c.id), crm: r ? { ok: r.ok, reason: r.reason, status: r.status } : null });
}));
app.put('/customers/:id', portalUser, wrap(async (req, res) => {
  const c = await customers.update(req.user, req.params.id, req.body || {});
  await store.audit(req.user.email, 'customer.update', c.id, c.company || c.contact);
  res.json({ ok: true, customer: c });
}));
app.post('/customers/:id/crm', portalUser, wrap(async (req, res) => {
  const c = await customers.get(req.user, req.params.id); if (!c) return res.status(404).json({ error: 'no such customer in your list' });
  const r = await customerCrm(c, req.user, await data.settings()); if (!r) return res.status(400).json({ error: 'dealer customers are not sent to the ADONI TECH CRM' });
  res.json({ ok: r.ok, reason: r.reason, status: r.status });
}));
...
app.post('/portal/quote', portalUser, wrap(async (req, res) => {
  ...
  if (req.user.role === 'dealer') {
    const t = dealers.terms(await dealers.get(req.user.email), await data.settings());
    const want = b.route === 'direct' ? 'direct' : 'resale';
    if (t.routes !== 'both' && t.routes !== want) return res.status(403).json({ error: `your dealer terms allow ${t.routes === 'resale' ? 'resale only' : 'direct supply only'} - contact ADONI TECH to change this` });
  }
  // a saved customer, or save the one typed in (skipped quietly when he is already in the list)
  let savedCustomer = null, crmNew = null;
  if (b.customer_id) {
    savedCustomer = await customers.get(req.user, b.customer_id); if (!savedCustomer) return res.status(404).json({ error: 'no such customer in your list: ' + b.customer_id });
    const { id, scope, owner, created_by, created_at, updated_at, updated_by, crm: _c, notes, ...cust } = savedCustomer;
    b.customer = { ...cust, customer_id: id };
  } else if (b.save_customer && b.customer) {
    try { savedCustomer = await customers.create(req.user, b.customer, await customerPrefix(req.user)); crmNew = await customerCrm(savedCustomer, req.user, await data.settings()); b.customer = { ...b.customer, customer_id: savedCustomer.id }; }
    catch (e) { if (e.status === 409 && e.existing) b.customer = { ...b.customer, customer_id: e.existing.id }; else if (e.status !== 400) throw e; }
  }
  ...
  for (const i of want) {
    ...
    const c = fn(r);
    items.push({ table: i.table, key: c.key, model: c.model, qty: Number(i.qty), remark: String(i.remark || '').slice(0, 200), ...(i.lug ? { lug: String(i.lug).slice(0, 60) } : {}), ...(i.wire ? { wire: String(i.wire).slice(0, 60) } : {}) });
  }
  try {
    const out0 = await createRfq({ line: 'portal', route: b.route, customer: b.customer, project: b.project || {}, items, message: b.message || '', formats: [], source: 'portal' }, req.user);
    res.json({ ...out0, customer_id: b.customer && b.customer.customer_id || null, customer_saved: !!(savedCustomer && !b.customer_id), customer_crm: crmNew ? crmNew.ok : null });
  } catch (e) { if (e.status) return res.status(e.status).json({ error: e.message }); throw e; }
}));
...
/* ------------------------------------------------------------------ admin: dealer commission register (direct supply) */
const COMMISSION_STATES = ['quoted', 'ordered', 'customer_paid', 'invoice_received', 'commission_paid', 'lost'];
app.get('/admin/commissions', admin, wrap(async (_q, res) => {
  const keys = await store.list('quotation/'); const out = [];
  for (const k of keys) { const q = await store.getJSON(k); if (!q || q.route !== 'direct' || !q.billing) continue;
    out.push({ id: q.id, number: q.number, rev: q.rev, date: q.date, sent_at: q.sent_at, dealer: q.issuer.company, dealer_email: q.issuer.dealer, dealer_code: q.issuer.code, customer: q.customer && (q.customer.company || q.customer.contact),
      currency: q.currency, customer_value: q.billing.customer_value, commission: q.billing.commission_value, adoni_net: q.billing.adoni_net, freight: q.freight || 0, status: (q.commission && q.commission.status) || 'draft', note: q.commission && q.commission.note, history: q.commission && q.commission.history }); }
  res.json({ states: COMMISSION_STATES, rows: out.sort((a, b) => b.number.localeCompare(a.number)) });
}));
app.patch('/admin/commission/:id', admin, wrap(async (req, res) => {
  const q = await quote.get(req.params.id); if (!q || q.route !== 'direct') return res.status(404).json({ error: 'no direct-supply quotation ' + req.params.id });
  const st = String((req.body || {}).status || ''); if (!COMMISSION_STATES.includes(st)) return res.status(400).json({ error: 'status must be one of ' + COMMISSION_STATES.join(', ') });
  if (st === 'commission_paid' && !['customer_paid', 'invoice_received'].includes(q.commission && q.commission.status)) return res.status(400).json({ error: 'commission is paid only after the customer has paid in full' });
  q.commission = q.commission || { history: [] };
  q.commission.status = st; q.commission.note = String((req.body || {}).note ?? q.commission.note ?? '').slice(0, 500);
  q.commission.history = [...(q.commission.history || []), { status: st, at: new Date().toISOString(), by: req.user.email }];
  await store.setJSON('quotation/' + q.id, q);
  await store.audit(req.user.email, 'commission.' + st, q.number, `${q.issuer.company} ${q.currency} ${q.billing ? q.billing.commission_value : ''}`);
  res.json({ ok: true, commission: q.commission });
}));
...
/* lead times: admin + settings leadtime.users (e.g. Shital) update them without touching the CSV */
const leadAuth = async (req, res, next) => {
  if (!req.user) return res.status(401).json({ error: 'login required' });
  if (req.user.role === 'admin') return next();
  const s = await data.settings(); const ok = String(s['leadtime.users'] || '').toLowerCase().split(/[,\s;]+/).filter(Boolean).includes(String(req.user.email).toLowerCase());
  ok ? next() : res.status(403).json({ error: 'lead times are updated by admin and the people in settings leadtime.users' });
};
const LT_VIEW = {
  shock_absorbers: r => ({ key: r.bk, model: r.model, series: r.series, group: r.group === 'crane' ? 'Crane buffers' : 'Shock absorbers' }),
  wire_rope_isolators: r => ({ key: r.model, model: r.model, series: 'AWRI-' + r.family_wire_mm, group: 'Wire rope isolators' }),
  rubber_mounts: r => ({ key: r.model, model: r.model, series: String(r.family || '').replace(/ cylindrical.*| shock absorber| isolator.*/i, ''), group: 'Rubber mounts' }),
  accessories: r => ({ key: r.code, model: r.name, series: r.kind || 'accessory', group: 'Accessories' }),
  other_products: r => ({ key: r.code, model: r.name, series: 'other', group: 'Other products' }),
};
app.get('/leadtimes', auth.optional, leadAuth, wrap(async (_q, res) => {
  const out = [];
  for (const t of data.LT_TABLES) {
    const [base, lt] = await Promise.all([data.rows(t, { base: true }), data.leadOverrides(t)]);
    for (const r of base) { if (r.status === 'obsolete') continue; const v = LT_VIEW[t](r), o = lt[v.key];
      out.push({ table: t, ...v, base_days: r.lead_time_days === '' ? null : Number(r.lead_time_days), days: o ? o.days : (r.lead_time_days === '' ? null : Number(r.lead_time_days)), override: o || null }); }
  }
  res.json(out);
}));
app.put('/leadtimes', auth.optional, leadAuth, wrap(async (req, res) => {
  const ch = ((req.body || {}).changes || []).slice(0, 2000); if (!ch.length) return res.status(400).json({ error: 'no changes' });
  const by = {}; for (const c of ch) (by[c.table] = by[c.table] || []).push(c);
  let n = 0; for (const [t, list] of Object.entries(by)) n += await data.setLeadTimes(t, list, req.user.email);
  await store.audit(req.user.email, 'leadtime.update', n + ' line(s)', ch.slice(0, 12).map(c => `${c.key}=${c.days === '' || c.days == null ? 'file' : c.days + 'd'}`).join(', ') + (ch.length > 12 ? ' …' : ''));
  res.json({ ok: true, changed: n });
}));

/* dealer rules: review, confirm every point, e-mail the policy letter (versioned) */
app.get('/admin/dealers/:email/policy', admin, wrap(async (req, res) => {
  const d = await dealers.get(req.params.email); if (!d) return res.status(404).json({ error: 'no such dealer' });
  const settings = await data.settings();
  res.json({ dealer: { email: d.email, company: d.company, status: d.status, code: d.code, country: d.country, territory: d.territory }, rules: dealers.rules(d, settings), policy: d.policy || null, history: (d.policy_history || []).map(h => ({ version: h.version, confirmed_at: h.confirmed_at, confirmed_by: h.confirmed_by, sent_at: h.sent_at, mail_ok: h.mail_ok, own: h.rules.filter(r => r.own).map(r => r.label) })) });
}));
app.post('/admin/dealers/:email/policy', admin, wrap(async (req, res) => {
  const b = req.body || {}; const settings = await data.settings();
  let d = await dealers.get(req.params.email); if (!d) return res.status(404).json({ error: 'no such dealer' });
  if (b.preview) {   // letter as it would go out, nothing saved
    const tmp = { ...d, rules: { ...(b.rules || {}) } }; ['discount_pct', 'max_discount_pct'].forEach(k => { tmp[k] = ''; });
    const r = await notify.dealerPolicy({ ...tmp, code: d.code || dealers.codeFrom(d.company) }, dealers.rules(tmp, settings), { preview: true, version: ((d.policy && d.policy.version) || 0) + 1, note: b.note });
    return res.json({ html: r.html });
  }
  const approving = b.approve && d.status !== 'approved';
  d = await dealers.confirmPolicy(req.params.email, b, req.user.email, settings);
  if (approving) d = await dealers.decide(req.params.email, 'approve', req.user.email, { code: b.code });
  const cc = String(settings['mail.dealer_cc'] || '').split(/[,\s]+/).filter(Boolean);
  const m = b.send === false ? null : await notify.dealerPolicy(d, dealers.rules(d, settings), { version: d.policy.version, date: d.policy.confirmed_at, note: d.policy.note, cc, signed: settings['quote.approver_name'] });
  if (m) await dealers.markPolicySent(d.email, m);
  await store.audit(req.user.email, 'dealer.policy.v' + d.policy.version, d.company, (approving ? 'approved + ' : '') + (m ? (m.ok ? 'mailed' : 'mail failed: ' + m.reason) : 'not mailed') + ' · own rules: ' + (Object.keys(d.rules || {}).join(', ') || 'none'));
  res.json({ ok: true, dealer: await dealers.get(d.email), mail: m ? m.ok : null, reason: m && !m.ok ? m.reason : undefined });
}));
