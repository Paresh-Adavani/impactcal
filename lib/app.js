...
async function createRfq(b, user) {
  ...
  const rfq = { id: idOf(number), number, created_at: new Date().toISOString(), status: 'new', line: b.line || 'shock',
    ...
    issuer, ...(dealer && b.route === 'direct' ? { route: 'direct' } : {}), selection: b.selection || null, items: b.items.map(i => ({ table: i.table || 'shock_absorbers', key: i.key || i.bk || i.model, model: i.model, qty: Number(i.qty) || 1, mounting: i.mounting || '', cap: i.cap || '', damping_code: i.damping_code || '', remark: i.remark || '' })),
