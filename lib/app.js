...
/** Create an RFQ (+ mails, CRM lead, draft quotation, approval alert). Shared by POST /rfq and "convert selection to RFQ". */
async function createRfq(b, user) {
  ...
  const rfq = { id: idOf(number), number, created_at: new Date().toISOString(), status: 'new', line: b.line || 'shock',
    customer: c, project: b.project || {}, raised_by: user ? { email: user.email, role: user.role, name: auth.displayName(user.email, settings) } : null,
    ...
    formats: b.formats || ['PDF'], message: b.message || '', lang: b.lang || 'en', from_selection: b.from_selection || null };
  await store.setJSON('rfq/' + rfq.id, rfq);
  await store.audit(user ? user.email : 'customer', 'rfq.create', number, rfq.line + (c.quality !== 'ok' ? ' · quality ' + c.quality : ''));
  ...
  return { id: rfq.id, number, mail: mails.sales, ack: mails.customer, quotation: q ? q.number : null, alert: alert && alert.mail ? alert.mail.ok : false, crm: lead ? { ok: lead.ok, reason: lead.reason, status: lead.status } : null, quality: c.quality };
}
