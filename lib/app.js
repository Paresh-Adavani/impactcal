...
app.get('/approve/:id', approveAuth, wrap(async (req, res) => {
  ...
  res.json({ quotation: q, rfq, drawings, bank: parties.bank, company: parties.company, drawings_mode: settings['drawings.mode'] || 'attach',
    access: req.access, actor: req.actor, limit: quote.limitCheck(q), billing: req.access !== 'sales' ? quote.dealerBilling(q) : null, offices,
...
app.post('/approve/:id/send', approveAuth, wrap(async (req, res) => {
  ...
  if (!isAdmin) {
    const chk = quote.limitCheck(q);
    if (!chk.ok) return res.status(403).json({ error: `Discount beyond your limit of ${chk.max_pct} % on ${chk.violations.map(v => v.model).join(', ')}. Raise the price or request a special price.`, limit: chk });
  }
...
app.post('/approve/:id/special', approveAuth, wrap(async (req, res) => {
  ...
  q.special = { status: 'pending', reason: String((req.body && req.body.reason) || '').slice(0, 1000), requested_by: who.name, requested_email: who.email, requested_at: new Date().toISOString(), asked: chk.violations };
  await quote.save(q);
