...
async function offerSent(q, settings, company, pdfBuffer, drawings, links, extras) {
  const c = q.customer || {}; const is = q.issuer || {};
  const direct = is.type === 'dealer' && q.route === 'direct';   // ADONI TECH offer raised by a channel partner
  const dealer = is.type === 'dealer' && !direct;
  const person = !dealer && !direct && is.person && is.person.email ? is.person : null;
  const ccList = dealer || direct ? [is.email, ...String(settings['mail.dealer_cc'] || settings['mail.approver_to'] || 'adonitech@gmail.com').split(/[,\s]+/)]
  ...
  for (const f of extras || []) attachments.push({ filename: f.filename, content: f.buffer });
  const signName = dealer || direct ? (q.approved_by || is.contact || is.company) : (q.approved_by || company.name || 'ADONI TECH');
  const signLines = dealer ? [is.company, [is.email, is.phone].filter(Boolean).join(' · '), is.web].filter(Boolean)
                  : direct ? [company.name, 'through our channel partner ' + is.company, [is.email, is.phone].filter(Boolean).join(' · ')].filter(Boolean)
  ...
  const body = `<p>Dear ${esc(c.contact || c.company || 'Sir/Madam')},</p>
    <p>Please find attached our quotation <b>${esc(q.number)}${q.rev ? ' rev ' + q.rev : ''}</b>${q.rfq_number && !dealer && !direct ? ` against your request ${esc(q.rfq_number)}` : ''}.</p>
    ...
    ${(extras && extras.length) ? `<p>Also attached: ${extras.map(f => esc(f.filename)).join(', ')}.</p>` : ''}
    <p>${esc(q.notes || '')}</p>
    <p>Regards,<br><b>${esc(signName)}</b><br>${signLines.map(esc).join('<br>')}</p>`;
  const smtpFrom = (require('./mail').config().from) || '';
  const from = dealer ? `${String(is.company || 'Dealer').replace(/[<>"]/g, '')} <${settings['mail.dealer_from'] || smtpFrom}>` : undefined;
  const replyTo = dealer || direct ? is.email : (person ? person.email : undefined);
...
async function dealerQuoteInternal(q, settings, billing) {
  ...
  if (billing.route === 'direct') {
    const body = `<p>Channel partner <b>${esc(is.company)}</b> (${esc(is.email)}, code ${esc(is.code)}) sent a <b>direct-supply</b> quotation <b>${esc(q.number)}</b> on ADONI TECH letterhead to <b>${esc(c.company || c.contact || '')}</b> (${esc(c.email || '')}).</p>
    <table>${row('List value', cur + ' ' + num(billing.list_value))}${row('Quoted by dealer (before his discount)', cur + ' ' + num(billing.quoted_value))}${row('Customer pays ADONI TECH (basic, before GST)', cur + ' ' + num(billing.customer_value))}
    ${row(`Commission to dealer (his ${billing.billing_discount_pct} % less what he passed on)`, cur + ' ' + num(billing.commission_value) + ' — payable after the customer pays in full')}${row('ADONI TECH net', cur + ' ' + num(billing.adoni_net))}
    ${billing.freight && billing.freight.amount ? row('Packing & freight line', cur + ' ' + num(billing.freight.amount) + ` (${billing.freight.kg_charged} kg × ${billing.freight.rate})`) : ''}</table>
    <table style="border-collapse:collapse;width:100%;margin-top:8px">${billing.lines.map(l => `<tr><td style="padding:4px;border-top:1px solid #E2E6EB"><b>${esc(l.model)}</b></td><td align="right" style="padding:4px;border-top:1px solid #E2E6EB">${l.qty}</td><td align="right" style="padding:4px;border-top:1px solid #E2E6EB">quoted ${num(l.quoted)}</td><td align="right" style="padding:4px;border-top:1px solid #E2E6EB">passed ${l.passed_pct} %</td><td align="right" style="padding:4px;border-top:1px solid #E2E6EB">commission ${l.commission_pct} % = ${num(l.commission)}</td></tr>`).join('')}</table>
    ${button(`${PUBLIC_URL()}/approve.html?id=${encodeURIComponent(q.id)}`, 'Open in ImpactCal')}`;
    return mail.send({ to, subject: `Direct-supply quotation ${q.number} via ${is.company} → ${c.company || c.contact || ''} — commission ${cur} ${num(billing.commission_value)}`, html: shell('Direct-supply quotation ' + q.number, body) });
  }
  const body = `<p>Dealer <b>${esc(is.company)}</b> (${esc(is.email)}, code ${esc(is.code)}) sent quotation <b>${esc(q.number)}</b> to <b>${esc(c.company || c.contact || '')}</b> (${esc(c.email || '')}).</p>
    ...
    ${row(`Our billing to dealer (list − ${billing.billing_discount_pct} %)`, cur + ' ' + num(billing.billing_value) + ' + GST')}${billing.freight && billing.freight.amount ? row('Packing & freight to his godown', cur + ' ' + num(billing.freight.amount) + ` (${billing.freight.kg_charged} kg × ${billing.freight.rate}) + GST`) : ''}${row('Dealer margin', cur + ' ' + num(billing.dealer_margin))}
...
/** 5. The dealer pricing policy letter.
 *  provisional: at sign-up - the standard policy, final terms confirmed by ADONI TECH on approval.
 *  confirmed:   after ADONI TECH has confirmed every point with the dealer (versioned). */
function dealerPolicyHtml(d, rules, opts = {}) {
  const v = k => (rules.find(r => r.k === k) || {}).value;
  const fmt = r => r.type === 'number' ? (/^Rs /.test(r.unit) ? `Rs ${r.value}${r.unit.slice(2)}` : `${r.value} ${r.unit === '%' ? '%' : r.unit}`) : r.type === 'select' ? (r.options[r.value] || r.value) : r.value;
  const D = Number(v('discount_pct')) || 25, P = Math.min(10, D / 2 >= 10 ? 10 : Math.floor(D / 2)), C = D - P;
  const routes = v('routes');
  const lines = rules.filter(r => r.value !== '' && r.value != null && !(r.k === 'commission' && routes === 'resale') && !(r.k === 'resale' && routes === 'direct'));
  const intro = opts.provisional
    ? `<p>Thank you for applying to become an ADONI TECH dealer. Below is our <b>standard dealer pricing policy</b> for your reference. We will review your application, go through each point with you, and then e-mail your <b>confirmed terms</b> together with your dealer code.</p>`
    : `<p>${opts.version > 1 ? 'Your dealer terms have been revised.' : 'Welcome to the ADONI TECH dealer network.'} As agreed with you, these are your <b>confirmed terms</b>${d.code ? ` (dealer code <b>${esc(d.code)}</b>)` : ''} — version ${opts.version}, ${new Date(opts.date || Date.now()).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}.${opts.version > 1 ? ' They replace the earlier version.' : ''}</p>`;
  const table = `<table style="border-collapse:collapse;width:100%;font-size:14px">${lines.map(r => `<tr><td style="padding:7px 8px;border-top:1px solid #E2E6EB;color:#5A6672;width:38%;vertical-align:top">${esc(r.label)}</td><td style="padding:7px 8px;border-top:1px solid #E2E6EB;vertical-align:top"><b>${esc(fmt(r))}</b></td></tr>`).join('')}</table>`;
  const ex = `<div style="background:#F7F9FC;border:1px solid #E2E6EB;border-radius:8px;padding:10px 14px;margin:14px 0;font-size:13.5px"><b>Example</b> — list price Rs 100, your dealer discount ${D} %:<br>
    ${routes !== 'direct' ? `• <b>Resale:</b> we invoice you Rs ${100 - D} (+ packing &amp; freight + GST). You sell at Rs ${100 - P} (${P} % off) → your margin Rs ${D - P}; or at Rs 125 (markup) → your margin Rs ${25 + D}.<br>` : ''}
    ${routes !== 'resale' ? `• <b>Direct supply:</b> you pass ${P} % → the customer pays us Rs ${100 - P} and your commission is ${C} % = Rs ${C}. You mark up to Rs 125 and pass ${P} % → your commission is ${C} % of 125 = Rs ${(C * 1.25).toFixed(2)}.` : ''}</div>`;
  const how = opts.provisional ? '' : `<p><b>How to quote:</b> sign in at ImpactCal with this e-mail address → Quote portal → pick products and quantities → choose resale or direct supply → set your discount or markup → send. A discount beyond your limit goes to us as a special-price request.</p>${button(`${PUBLIC_URL()}/portal.html`, 'Open the dealer portal')}`;
  return `<p>Dear ${esc(d.contact || d.company)},</p>${intro}${table}${ex}${opts.note ? `<p>${esc(opts.note)}</p>` : ''}${how}
    <p style="color:#5A6672;font-size:12.5px">Any change to these terms is confirmed to you in writing by e-mail. Please reply to this mail if anything differs from what we discussed.</p>
    <p>Regards,<br><b>${esc(opts.signed || 'Paresh Adavani')}</b><br>ADONI TECH</p>`;
}
async function dealerPolicy(d, rules, opts = {}) {
  const html = shell(opts.provisional ? 'Dealer pricing policy — standard terms' : `Your dealer terms — version ${opts.version}`, dealerPolicyHtml(d, rules, opts));
  if (opts.preview) return { ok: true, preview: true, html };
  const r = await mail.send({ to: d.email, cc: opts.cc || [], subject: opts.provisional ? `ADONI TECH dealer pricing policy — thank you for applying, ${d.company}` : `ADONI TECH dealer terms${d.code ? ' — ' + d.code : ''} (version ${opts.version})`, html });
  return { ...r, html };
}
...
module.exports = { dealerPolicy, dealerPolicyHtml, dealerQuoteInternal, draftReady, specialRequested, specialDecided, dealerApplied, dealerApproved, rfqReceived, selectionDownloaded, approvalRequested, offerSent, otp, whatsapp, PUBLIC_URL };
