...
/* ------------------------------ settings --------------------------------- */
const MAIL_LABEL={'mail.enabled':'Send mail on a new RFQ (1 = yes, 0 = no)','mail.host':'SMTP host',
 'mail.port':'SMTP port','mail.secure':'SSL (1 = yes)','mail.user':'SMTP user / Gmail address',
 'mail.pass':'App password','mail.from':'From address (blank = SMTP user)','mail.to':'Send enquiries to'};
...
async function loadSettings(){
  ...
  $('#mailForm').innerHTML=Object.entries(MAIL_LABEL).map(([k,l])=>
    `<div><label>${l}</label><input data-k="${k}" ${k==='mail.pass'?'type="password" placeholder="leave blank to keep the saved one"':''} value="${esc(s[k]??'')}"></div>`).join('');
...
$('#setSave').onclick=async()=>{
  const body={}; $$('#setForm [data-k], #mailForm [data-k]').forEach(i=>body[i.dataset.k]=i.value);
...
$('#mailVerify').onclick=async()=>{
  $('#mailMsg').innerHTML=`<div class="note blue">Checking the connection…</div>`;
  const r=await api('/api/mail/verify');
  $('#mailMsg').innerHTML=r.ok
    ? `<div class="note ok">Connected to ${esc(r.host)}:${r.port} as ${esc(r.user)}.</div>`
    : `<div class="note bad">${esc(r.reason)}</div>`;
};
$('#mailTest').onclick=async()=>{
  $('#mailMsg').innerHTML=`<div class="note blue">Re-sending the most recent RFQ…</div>`;
  const r=await api('/api/mail/test',{method:'POST'});
  $('#mailMsg').innerHTML=r.ok
    ? `<div class="note ok">Sent to ${r.sent.map(esc).join(', ')}.</div>`
    : `<div class="note bad">${esc(r.reason)}</div>`;
};
