...
async function rfq() {
  ...
  ${rows.map(r => `<tr><td class="mono">${esc(r.number)}</td><td>${dt(r.created_at)}</td><td><b>${esc(r.customer || '')}</b><br><small class="muted">${esc(r.country || '')}</small></td><td>${esc(r.line)}</td><td class="n">${r.items}</td><td><small>${esc(r.raised_by || 'web')}</small></td><td><span class="pill ${r.status === 'quoted' ? 'ok' : r.status === 'lost' ? 'bad' : 'brand'}">${esc(r.status)}</span></td><td class="mono">${r.quotation_id ? r.quotation_id.replace(/-/g, '/') : '—'}</td>
    <td class="row">${r.quotation_id ? `<button class="btn sm" data-open="${esc(r.quotation_id)}">Open / approve</button>` : `<button class="btn sm navy" data-draft="${esc(r.id)}">Prepare quotation</button>`}<button class="btn sm ghost" data-view="${esc(r.id)}">Details</button>${AUTH.user.role === 'admin' ? `<button class="btn sm ghost" data-del="${esc(r.id)}" data-n="${esc(r.number)}" style="color:#B42318">Delete</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="9" class="muted">No requests yet.</td></tr>'}
  ...
  $$('[data-del]').forEach(b => b.onclick = async () => { if (!confirm(`Delete ${b.dataset.n} permanently?\n\nUse this only for trial / test records. The RFQ and its draft quotation are removed for good (the deletion is logged in Team & activity).`)) return; try { await api('admin/rfq/' + encodeURIComponent(b.dataset.del), { method: 'DELETE' }); show(); } catch (e) { alert(e.message); } });
...
async function selections() {
  ...
  ${rows.map(r => `<tr><td class="mono">${esc(r.number)}</td><td>${dt(r.created_at)}</td><td><b>${esc(r.contact || '')}</b><br><small class="muted">${esc(r.email || '')} · ${esc(r.phone || '')}</small></td><td>${esc(r.customer || '')}<br><small class="muted">${esc(r.country || '')}</small></td><td>${esc(r.line)}</td><td class="mono">${esc(r.model || '')}</td><td class="n">${r.qty || ''}</td><td><span class="pill ${r.quality === 'ok' ? 'ok' : 'bad'}">${esc(r.quality || '?')}</span></td><td>${r.crm ? '✓' : '—'}</td><td class="mono">${esc(r.rfq_number || '—')}</td>
    <td class="row">${r.rfq_number ? '' : `<button class="btn sm navy" data-conv="${esc(r.id)}">Convert to RFQ</button>`}<button class="btn sm ghost" data-sview="${esc(r.id)}">Details</button>${AUTH.user.role === 'admin' ? `<button class="btn sm ghost" data-sdel="${esc(r.id)}" data-n="${esc(r.number)}" style="color:#B42318">Delete</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="11" class="muted">No report downloads yet.</td></tr>'}
  </tbody></table></div></div><div id="detail"></div>`;
  ...
  $$('[data-sdel]').forEach(b => b.onclick = async () => { if (!confirm(`Delete selection record ${b.dataset.n} permanently?\n\nUse this only for trial / test downloads. A linked RFQ (if any) is kept. The deletion is logged in Team & activity.`)) return; try { await api('admin/selection/' + encodeURIComponent(b.dataset.sdel), { method: 'DELETE' }); show(); } catch (e) { alert(e.message); } });
...
async function quotes() {
  ...
  ${rows.map(q => `<tr><td class="mono">${esc(q.number)}</td><td>${q.rev}</td><td>${q.date}</td><td>${esc(q.customer || '')}</td><td><small>${esc(String(q.issuer || 'adoni').replace(/^adoni:?/, 'ADONI TECH ').replace(/^dealer:/, 'Dealer: ').replace(/^direct:/, 'Direct via '))}</small>${q.special ? ` <span class="pill ${q.special === 'approved' ? 'ok' : q.special === 'pending' ? 'brand' : 'bad'}">special ${esc(q.special)}</span>` : ''}</td><td>${q.currency}</td><td class="n">${money(q.grand_total, q.currency)}</td><td><span class="pill ${q.status === 'sent' ? 'ok' : 'brand'}">${q.status}</span></td><td>${dt(q.sent_at)}</td>
    <td class="row"><button class="btn sm" data-open="${esc(q.id)}">Open</button><a class="btn sm ghost" href="/api/admin/quotation/${encodeURIComponent(q.id)}/pdf?token=${encodeURIComponent(AUTH.token)}" target="_blank">PDF</a>${AUTH.user.role === 'admin' ? `<button class="btn sm ghost" data-qdel="${esc(q.id)}" data-n="${esc(q.number)}" style="color:#B42318">Delete</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="10" class="muted">None yet.</td></tr>'}</tbody></table></div></div>`;
  $$('[data-open]').forEach(b => b.onclick = async () => { const q = await api('admin/quotation/' + encodeURIComponent(b.dataset.open)); location.href = q.approve_url; });
  $$('[data-qdel]').forEach(b => b.onclick = async () => { if (!confirm(`Delete quotation ${b.dataset.n} permanently?\n\nUse this only for trial / test quotations. Its RFQ (if any) goes back to status "new". The deletion is logged in Team & activity.`)) return; try { await api('admin/quotation/' + encodeURIComponent(b.dataset.qdel), { method: 'DELETE' }); show(); } catch (e) { alert(e.message); } });
}
