...
// Serve the app files with revalidation so an update is never hidden behind a
// stale browser cache. Images are content-addressed by name and rarely change,
// so they keep a long cache.
app.use(express.static(path.join(__dirname, 'public'), {
  etag: true, lastModified: true,
  setHeaders(res, filePath) {
    res.setHeader('Cache-Control', /\.(html|css|js)$/i.test(filePath)
      ? 'no-cache'                       // always revalidate — cheap, 304 when unchanged
      : 'public, max-age=86400');        // pictures
  },
}));
...
app.post('/api/rfq', wrap((req, res) => {
  ...
  // Notify sales. A dead mail server must never lose the enquiry.
  mail.sendRfq(rid)
    .then(m => res.json({ id: rid, number, mail: m }))
    .catch(e => res.json({ id: rid, number, mail: { ok: false, reason: e.message } }));
}));
...
/* -------------------------------- settings ------------------------------- */
const SECRET = new Set(['mail.pass']);
app.get('/api/settings', admin, wrap((_q, r) => r.json(Object.fromEntries(
  db.prepare('SELECT k,v FROM setting').all()
    .map(x => [x.k, SECRET.has(x.k) ? (x.v ? '••••••••••••' : '') : x.v])))));
app.put('/api/settings', admin, wrap((req, res) => {
  db.transaction(() => {
    for (const [k, v] of Object.entries(req.body || {})) {
      // a masked or blank secret means "leave it alone"
      if (SECRET.has(k) && (v === '' || /^[•*]+$/.test(String(v)))) continue;
...
/* ---------------------------------- mail --------------------------------- */
app.get('/api/mail/verify', admin, (req, res) => {
  mail.verify().then(r => res.json(r)).catch(e => res.json({ ok: false, reason: e.message }));
});
app.post('/api/mail/test', admin, (req, res) => {
  const last = db.prepare('SELECT id FROM rfq ORDER BY id DESC LIMIT 1').get();
  if (!last) return res.json({ ok: false, reason: 'No RFQ yet to use as a sample. Send one from the selector first.' });
  mail.sendRfq(last.id).then(r => res.json(r)).catch(e => res.json({ ok: false, reason: e.message }));
});
app.get('/api/audit', admin, wrap((_q, r) => r.json(db.prepare('SELECT * FROM audit ORDER BY id DESC LIMIT 300').all())));

const PORT = process.env.PORT || 5000;
if (require.main === module) app.listen(PORT, () => console.log(`ImpactCal on http://localhost:${PORT}`));
module.exports = app;
